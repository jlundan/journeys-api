...
func ExtractRoutesByAgencies(input *csv.Reader, output *csv.Writer, agencies map[string]struct{}) (map[string]struct{}, []error) {
	errs := make([]error, 0)
	...
	if agencyIdPos, hasAgencyId := headers["agency_id"]; hasAgencyId {
		...
		errs = append(errs, createFileError(RoutesFileName, "cannot extract routes without agency_id column"))
		return nil, errs
	}

	var routeIdPos uint8
	if pos, hasRouteId := headers["route_id"]; hasRouteId {
		routeIdPos = pos
	} else {
		errs = append(errs, createFileError(RoutesFileName, "cannot extract routes without agency_id column"))
		return nil, errs
	...
	for {
		...
		if rErr != nil {
			errs = append(errs, createFileError(RoutesFileName, fmt.Sprintf("%v", rErr.Error())))
			continue
		}
		...
		if _, shouldBeExtracted := agencies[row[idHeaderPos]]; shouldBeExtracted {
			...
			route := row[routeIdPos]
			if _, routeAlreadyExists := routeMap[route]; !routeAlreadyExists {
