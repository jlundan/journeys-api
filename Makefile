...
coverage:
	JOURNEYS_BASE_URL=http://localhost:5678 JOURNEYS_VA_BASE_URL="https://data.itsfactory.fi/journeys/api/1" go test -count=1 -tags=all_tests -coverpkg=./... -coverprofile cover.out ./...

coverage-html:
	JOURNEYS_BASE_URL=http://localhost:5678 JOURNEYS_VA_BASE_URL="https://data.itsfactory.fi/journeys/api/1" go test -count=1 -tags=all_tests -coverpkg=./... -coverprofile cover.out ./... && go tool cover -html=cover.out -o coverage.html
tre:
...
tre-dev:
	JOURNEYS_GTFS_PATH=.gtfs JOURNEYS_PORT=5678 JOURNEYS_BASE_URL=http://localhost:5678/v1 JOURNEYS_VA_BASE_URL="https://data.itsfactory.fi/journeys/api/1" go run -race cmd/journeys/journeys.go --disable-cache
tre-dev-no-val:
	JOURNEYS_GTFS_PATH=.gtfs JOURNEYS_PORT=5678 JOURNEYS_BASE_URL=http://localhost:5678/v1 JOURNEYS_VA_BASE_URL="https://data.itsfactory.fi/journeys/api/1" go run -race cmd/journeys/journeys.go --disable-cache --skip-validation
vet:
	go vet ./...