...
test-v:
	CGO_ENABLED=0 go test --count=1 -v -tags=all_tests ./...

coverage:
	CGO_ENABLED=0 go test -count=1 -tags=all_tests -coverprofile cover.out ./...

coverage-html:
	CGO_ENABLED=0 go test -count=1 -tags=all_tests -coverprofile cover.out ./... && go tool cover -html=cover.out -o coverage.html
tre:
